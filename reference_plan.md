...
- `scripts/import_reference_recall_reviews.py` imports complete or partial reviewer JSONL into a separate review database. It validates source identity, live text hashes and absolute spans, records negative/ambiguous/unresolved findings, and only marks a finding `eligible_reviewed` when local resolution and confidence/duplicate gates pass. All 200 pilot rows have now been independently reviewed in 21 batches, producing 1,116 exact findings: 115 duplicate relationships, six duplicate occurrences, 26 ambiguous targets, 951 unresolved targets and 18 eligible reviewed findings. The 18 eligible findings include the unique `FCA Handbook` finding and later findings promoted through the guarded stage. No unresolved reviewer finding was guessed into the graph.
...
- Reviewer outputs for all 200 rows are retained under `logs/reference-recall-review-outputs-20260731/`. A second-adjudication pass grouped the 26 ambiguous cases into structural collisions, glossary variants and two-target legal citations; they remain held because no unique canonical target was established. Three additional high-confidence lexical candidates (an external BIC/ISO identifier and two ambiguous Part/rule labels) were checked but likewise not promoted. The pilot gate is complete; the next operational step is to review the remaining corpus queue in stratified batches and adjudicate only findings with a unique canonical target.

## Implementation status (corpus-wide completion, 2026-07-31)

The pilot was not the final review. The remaining corpus queue has now been processed in one deterministic, auditable pass using `scripts/review_reference_recall_corpus.py`. Every candidate in `logs/reference-recall-ledger-corpus-final-20260731.sqlite3` has an explicit disposition in `logs/reference-recall-corpus-review-final-20260731.sqlite3`; no candidate is left without a review row.

- Scope: 29,693 source provisions and 106,558 ledger candidates. The final ledger run is scanner version 2 (`20260731T203819Z-b55080e7`) and reports 56,846 candidates covered by an edge, 8,552 by an occurrence, 589 by a prior LLM result, 65 classified as not references, 12,288 excluded context, 5,934 still needing review under the scanner taxonomy, 3,987 prior LLM findings unresolved, and 18,297 tail candidates. Those last three ledger labels are scanner queues; the corpus review database records the explicit disposition for each of them.
- Corpus review decisions: 65,989 already covered, 12,973 genuine references that are external or currently unresolved to a local node, 15,243 ambiguous/held (12,705 needing structural context, 2,454 with multiple local targets and 84 unsupported candidate kinds), and 12,353 not references (12,288 excluded context plus 65 not applicable). The counts sum to all 106,558 candidates.
- Safe materialisation added 499 `references` edges and 1,826 exact `REF` occurrences in two guarded passes (270/1,073 before the structural-index fix and 229/753 after it). The fix preserves punctuation in structural identifiers such as `2.4`; without it, unique local targets could be mistaken for ambiguity. The final recovery totals are 1,481 `references` edges, 31 `uses_defined_term` edges, 2,946 `REF` occurrences and 31 `DEF` occurrences for `reference_recall_stage_v1`.
- The source database passed `PRAGMA quick_check` and `PRAGMA foreign_key_check`. Recovery edges have zero self-edges or missing targets; recovery occurrences have zero orphan edges, zero duplicate source-target-span rows and zero non-exact/invalid source spans (using the same title fallback as the scanner). The staged output is now empty after the final post-apply review, confirming there are no further unique local candidates waiting for this pass.

The remaining external/unresolved and ambiguous findings are deliberately retained as explicit, auditable holds rather than guessed into the graph. They are the residual adjudication queue for future targeted improvements, not an unreviewed corpus tail. The materialisation audits are `logs/reference-recall-corpus-materialize-20260731.json` and `logs/reference-recall-corpus-materialize-final-20260731.json`.
